      ...
      let :params do
        {
          ...
          :nginx_syslog_access_server => '127.0.0.1',
          :nginx_syslog_error_server => '10.10.10.10',
          :nginx_syslog_access_tag => 'rpaas01',
          :nginx_syslog_error_tag => 'rpaas01error',
...
EOF
      ...
      it 'custom syslog server' do
        ...
        should contain_file('/etc/nginx/nginx.conf').with_content(/\s+error_log\ syslog:server=10.10.10.10,facility=local6,tag=rpaas01error;/)
      end
    ...
    let :params do
      {
        ...
        :nginx_syslog_error_server  => 'localhost'
      }
    end
...
EOF
    ...
    context 'using json log format' do
      let :params do
        {
          :nginx_request_id_enabled   => true,
...
EOF
      ...
      it 'generate custom json access_log' do
        should contain_file('/etc/nginx/nginx.conf').with_content(/#{Regexp.escape(log_format)}/m)
      end
    end
  end
