...
function _M.update_ticket_encryption_key(key, nkeys)
    ...
    -- OpenSSL session ticket key is 48 bytes or 80 bytes.
    -- key structure:
    -- 16 bytes key name, 16 (or 32) bytes AES key, 16 (or 32) bytes HMAC key.
...
function _M.update_last_ticket_decryption_key(key)
    ...
    -- OpenSSL session ticket key is 48 or 80 bytes.
    if not key or #key ~= 48 or #key ~= 80 then
        return nil, 'invalid ticket key'
    end
