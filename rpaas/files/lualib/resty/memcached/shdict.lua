...
function _M.gen_memc_methods (opts)
    ...
    local function init_memc (ctx)
        ...
        if DEBUG then error_log(ctx, "connected to memc") end
    ...
    local function fetch_key_from_memc(ctx, key)
        ...
        if not disable_shdict then
            ...
            if elapsed then
                ...
                if cached and not stale then
                    if DEBUG then
                        error_log(ctx, "some other timer just inserted a fresh memc "
        ...
        error_log(ctx, "trying to get ".. key .." from memcached - by paulo with ")
	if res then
             error_log(ctx, "getting: " .. res)
        end
        if not res and err then
            ...
            for i = 1, fetch_retries do
                ...
                if fetch_retry_delay > 0 then
                    if DEBUG then
                        error_log(ctx, "waiting for ", fetch_retry_delay,
                              " sec before the next retry #", i)
                    end
                    sleep(fetch_retry_delay)
                end

                error_log("retrying fetch #", i, " from ", tag,
                      ' due to error "', err, '"')
        ...
        if not ok and err then
            ...
            for i = 1, store_retries do
                ...
                if store_retry_delay > 0 then
                    if DEBUG then
                        error_log(ctx, "waiting for ", store_retry_delay,
                              " sec before the next retry #", i)
                    end
                    sleep(store_retry_delay)
                end

                error_log("retrying store #", i, " from ", tag,
                      ' due to error "', err, '"')
